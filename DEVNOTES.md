...
- **DI 사이클 회귀 검증 갭** — 2026-05-04 bc65b20(M-15)에서 도입된 `RewardDispatcher↔ExpService` 순환 의존이 2026-05-06 부팅 시점에야 표면화(8 커밋 지연). 원인: (1) `DiSmokeTests`가 `services.BuildServiceProvider()`를 `ValidateOnBuild`/`ValidateScopes` 없이 호출 + `AddContentServices()` 미호출로 사이클이 아예 형성되지 못해 정적 검증 불가, (2) qa-reviewer 루틴이 빌드/xUnit까지만 검증하고 `dotnet run` 부팅 검증 미포함이라 `WebApplication.CreateBuilder`의 자동 `ValidateOnBuild` 미실행. 처치 후보: 작은 안 — `DiSmokeTests`에 `ServiceProviderOptions { ValidateOnBuild = true, ValidateScopes = true }` 적용 + 모든 `Add*Services`/`Add*Repositories` 호출 보강(CI 정적 검출 가능). 큰 안 — qa-reviewer 워크플로에 Framework.Api 부팅 health 체크 단계 추가. 본 라운드(2026-05-06)는 표면 회귀(`ExpService` 의존성 역전)만 해소, 검증 갭은 별도 라운드. **2026-05-07 진행**: 1단계 단위 테스트(ExpService 10개, AuthService 16개) 추가 완료 — 서비스 로직 회귀는 커버됨. `DiSmokeTests` `ValidateOnBuild` 미적용 및 Google Login 등 AuthService 나머지 메서드 테스트는 미완, 별도 라운드 유지.
...
- ~~**`IapPurchaseService.ExecuteConsumeAsync` 동시성 처리**~~ — **[해결]** try/catch(DbUpdateConcurrencyException) + 1회 재시도 + 재로드 가드 적용. 성공/실패 양 경로 모두 처리. verify 본 경로(3회)와 달리 retry 워커가 안전망이므로 최소 패턴 적용, 재시도 실패 시 예외 전파 없이 LogError 후 종료.
