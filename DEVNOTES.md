...
## [미구현] 추가 개발 필요 항목
- **플레이어 인벤토리 조회 API** — `GET /api/items/inventory` 엔드포인트 미구현. `IPlayerItemService.GetByPlayerIdAsync` 서비스는 존재, `PlayerItemDto` DTO 정의됨. `Controllers/Player/ItemsController.cs` 추가만 필요
